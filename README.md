...
1. **Onboarding Flow** ✅ IMPLEMENTED
   ...
   - Language selection with native script display (العربية, Yorùbá, etc.)
...
4. **Lesson Viewer**
   - Click topic → Load lesson content
   - Lesson structure:
...
5. **Quiz System**
   ...
   - Incorrect answers → Personalized explanation (not just "wrong")
   - Mastery calculation: (correct/total) × 100
...
│   ├── lib/
│   │   ├── llama/
│   │   │   ├── prompts.ts (all Llama prompt templates)
...
│   │   ├── utils/
│   │   │   ├── education-systems.ts (country→grade mapping)
│   │   │   ├── language-codes.ts
...
2. Sequence topics by prerequisite logic (foundational → advanced)
  ...
  "examples": [
    {
      ...
      "steps": ["Divide 500 by 4", "500 ÷ 4 = 125"]
    }
  ],
  "practiceProblems": [
    {
